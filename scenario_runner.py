...
class ScenarioRunner(object):
    ...
    def _analyze_scenario(self, config):
        ...
        if self._args.json:
            # print("------------------------check print out the file name end with json")
    ...
    def _record_criteria(self, criteria, name):
        ...
        # Wei: the criteria is a list, so obvious it is from the 
        # get_criteria(extract_nodes_from_tree(criteria_tree)) function in basic_scenario.py
        k=0
        # print("***********************************************************")
        ...
        with open('temp.json', 'w', encoding='utf-8') as fp:
            ...
            for criterion in criteria:
                ...
                # print("########################### criterion_dict is:",criterion_dict)
                ...
                for key in criterion_dict:
                    ...
                    if key != "name":
                        try:
                            if key == "actor" or key == "other_actor":
                                #key_dict = {key: criterion_dict[key].type_id}
                                key_dict = {key: criterion_dict[key].attributes}
                                #wei: actor 的attributes 里面是有 role_name 的，可以根据这个判断是谁被撞了
                                # criteria_dict[criterion.name][key] = criterion_dict[key].type_id
                            else:
                                key_dict = {key: criterion_dict[key]}

                            # print(",,,,,,,,,,,,,,,,, key dict is:",key_dict) #所以是没办法dump 的就留下了？
                            json.dump(key_dict, fp, sort_keys=False, indent=4)
                            #但是dump的时候就缺少了很多
                            criteria_dict[criterion.name].update(key_dict)
                            # print(".......................criteria_dict[criterion.name] is:",criteria_dict[criterion.name],criterion.name)
                            # print("+++++++++++++++++++++ criteria_dict is:",criteria_dict[criterion.name])
                            #wei: 这里的criterion.name 是越来越多的, 得弄清楚 update 函数在这里为什么没有update 对应的值
                            
                            # 懂了！！！！先用temp.json 来dump的原因是为了试出来哪些可以dump 哪些不可以！！！                            
    ...
    def _load_and_run_scenario(self, config):
        ...
        try:
            ...
            if self._args.record:
                recorder_name = "{}/{}/{}.log".format(
                    os.getenv(os.path.dirname(os.path.abspath(__file__)), "./"), self._args.record, config.name)
            
            # if self._args.record:
            #     recorder_name = "{}/{}/{}.log".format(
            #         os.getenv(os.path.dirname(os.path.abspath(__file__))), self._args.record, config.name)
            # Load scenario and run it
            #wei: here the scenario we use is openscenario, which herit the basic scenario. 
            # self.manager.scenario(below part) = Openscenario.scennario. which is the self.scenario in basic scenario
            #Openscenario is herit from the basic scenario
            #the self.scenario in basicscenario is initialized by class Scenario in the basic_scenario.py    
            ...
            if self._args.record:
                ...
                # wei: 看起来是最后运行完之后将criteria 中存储的变量给update 到record_criteria 的变量中去。所以问题就变成了，criteria 中的update 函数为什么没有及时的修改变量。
