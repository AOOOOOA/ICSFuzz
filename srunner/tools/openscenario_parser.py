...
class OpenScenarioParser(object):
    ...
    @staticmethod
    def convert_condition_to_atomic(condition, actor_list,other_actors):
        """
        ...
        """
        #wei: 这里是有actor 信息的，为什么没有被传进npc test呢
        # print("CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCcall the convert_condition_to_atomic function")
        # print("+++++++++++++++++++++++++++++++++ other_actors in the convert_condition_to_atomic function",other_actors,other_actors[0].attributes,len(other_actors))
        atomic = None
        delay_atomic = None
        ...
        #TODO:这个地方要好好调试一下trigger_actor, triggered_actor, ego_actor和 other_actor 之间的联系
        if condition.find('ByEntityCondition') is not None:
        
            # wei: the byentity condition is not None! open_scenario.py 中的两个函数调用了它
            # print("22222222222222222222222222222 by entity condition is not none ")
            ...
            for triggering_entities in condition.find('ByEntityCondition').iter('TriggeringEntities'):
                for entity in triggering_entities.iter('EntityRef'): #here the entity ref is the hero, so how to add the npc?
                    for actor in actor_list:
            ...
            for entity_condition in condition.find('ByEntityCondition').iter('EntityCondition'):
                ...
                #     #TODO: 这个没有在by entity condition 中出现（followingleadingvehicle)，所以如何在xosc中定义并使用collision condition 呢？
                #     print('collisionCondition')
                ...
                #         atomic = atomic_cls(trigger_actor, terminate_on_failure=True, name=condition_name)   
                    ...
                    if collision_condition.find('EntityRef') is not None:
                        collision_entity = collision_condition.find('EntityRef')
                        for actor in actor_list:
                            if actor.attributes['role_name'] == "adversary":
                                triggered_actor = actor
                                break
                        ...
                        if triggered_actor is None:
                            raise AttributeError("Cannot find actor '{}' for condition".format(
                                collision_condition.attrib.get('entityRef', None)))
                        # print(")))))))))))))))))))))) other actor is:",triggered_actor)

                        atomic_cls = py_trees.meta.inverter(CollisionTest)
                        atomic = atomic_cls(actor=trigger_actor, other_actor=triggered_actor, terminate_on_failure=True,
                                            name=condition_name)
                ...
                #################################################################################################
                #####move the other actor part to the end of the condition analysis #############################
                #################################################################################################

                #TODO: try to add the other actor analysis part here. should we have two collision condition or one?
                #FIXME: here the collisioncondition should be modified
                elif entity_condition.find('CollisionCondition') is not None:
                    trigger_actor=None #re-set the actor to none in order to remove the impact from the ego vehicle 
                    try:
                        ...
                        print("cannot find the npc vehicle and cannot execute the collision monitor")
                        break
                    collision_condition = entity_condition.find('CollisionCondition')
                    ...
                    if collision_condition.find('EntityRef') is not None:
                        ...
                        if triggered_actor is None:
                            raise AttributeError("Cannot find NPC actor for condition")
                        # print("))))))))))))))))))))))1 other actor is:",triggered_actor)
                        atomic_cls = py_trees.meta.inverter(NpcTest)
                        atomic = atomic_cls(actor=trigger_actor, other_actor=triggered_actor, terminate_on_failure=True,
                                            name=condition_name)
                        ...
                        triggered_type = OpenScenarioParser.actor_types[collision_type]
                        # print("))))))))))))))))))))))2 other actor type is:",triggered_type)
        ...
        #TODO: 所以正经的有检测的部分是这里，我刚才一直改错了。 
        elif condition.find('ByValueCondition') is not None:
            # print("88888888888888888888 actor list is in atomic function", actor_list)
            npc_test=False
            ...
            if value_condition.find('ParameterCondition') is not None:
                ...
                if condition_name.startswith('criteria_'):
                    ...
                    #wei: just curious whether the globals() contain the collisionTest name
                    # print("ggggggggggggggggggggsgggggggglobals checking:",globals())
                    # print("-----------------------------------------------------------------")
                    if class_name == 'NpcTest':
                        # print("00000000000000000enter NPC test ")
                        npc_test=True
                    if class_name == 'CollisionTest':
                        collision_test= True
                        
                    if class_name in globals():
                        criterion_instance = globals()[class_name]
                        #wei: result is that the testing instance is created here instead in the above by entity condition
                        if class_name =="NpcTest":
                            pass
                            # print("corresponding criterion instance is:",criterion_instance)
                    else:
                        raise AttributeError(
                            "The condition {} cannot be mapped to a criterion atomic".format(class_name))
                    #FIXME: why it is for multiple ego vehicles?
                    atomic = py_trees.composites.Parallel("Evaluation Criteria for multiple ego vehicles")
                    # when using the creat_test_criteria, the actor list here only contain the ego vehicle 
                    if collision_test == True or npc_test == True:
                        if collision_test == True:
                            trigger_actor= actor_list[0]
                            triggered_actor=other_actors[0]
                            collision_test=False
                            if arg_name != '':
                                atomic.add_child(criterion_instance(trigger_actor,triggered_actor, arg_value))
                            else:
                                # print("NPC test Alone: triggered actor is when arg_name == NONEEEEEEEEEEEEE:",triggered_actor)                            
                                atomic.add_child(criterion_instance(trigger_actor, triggered_actor))
                        
                        elif npc_test== True:
                            trigger_actor = other_actors[0] #TODO: if in the future we have multiple actors, here should be modified
                            triggered_actor=actor_list[0]
                            npc_test=False
                        ...
                        for triggered_actor in actor_list:
                            if arg_name != '':
                                # print("triggered actor is when arg_name != none:",triggered_actor)
                                atomic.add_child(criterion_instance(triggered_actor, arg_value))
                            else:
                                # print("triggered actor is when arg_name == NONEEEEEEEEEEEEE:",triggered_actor)
