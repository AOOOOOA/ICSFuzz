...
class OpenScenario(BasicScenario):
    ...
    def _create_condition_container(self, node, story, name='Conditions Group', sequence=None,
        ...
        """
        #wei: this function is called by 1. condition group 2. start criteria 3. stop criteria
        #FIXME：but in reality it is not stop even the stop trigger fulfilled
        ...
        for condition_group in node.iter("ConditionGroup"):
            ...
            for condition in condition_group.iter("Condition"):
                # print("8888888888888888888888888 create_condition_contained called")
                # print("88888888888888888888888888 other actors are",self.other_actors[0].attributes)
                
                criterion = OpenScenarioParser.convert_condition_to_atomic(
                    condition, self.other_actors + self.ego_vehicles,self.other_actors)
                #wei: the convert condition to atomic is called here
    ...
    def _create_test_criteria(self):
        ...
        """
        #FIXME: when create_test_criteria的时候，actor变成了只有ego vehicle
        # print("9999999999999999999999999999999999 create test criteria called")
        #wei: 实际上这个函数也是被调用了的，但是我没有找到具体调用这个方程的地方。 
        ...
        #wei: 这里的condition 指的stop trigger 里的condition

        for condition in criteria:
            criterion = OpenScenarioParser.convert_condition_to_atomic(condition, self.ego_vehicles,self.other_actors)
            #wei: the convert condition to atomic is called here
...
#TODO: to fix the log error-show problem. 1. check the self.other_actors is right or not. 2,check exactly how the convert_condition_to_atomic function

        """
        这是scenario_runner 中关于create_test_scenario 的说明，所以说我暂时不用去找究竟是哪个文件/函数调用了它，有可能他就是用了py_trees 中的自动实现
