...
class CollisionTest(Criterion):
    ...
    def __init__(self, actor, other_actor=None, other_actor_type=None,
        ...
        #wei: 如果想在json 文件中加一些内容的话直接在init 这里加变量就可以了
        ...
        #why the collision sensor in the json log always shown as none?--fixed
        ...
        self.collision_event_dict=[]
        self.collision_with_other_actor = False
    ...
    def update(self):
        ...
        self.logger.debug("%s.update()[%s->%s]" % (self.__class__.__name__, self.status, new_status))
        # print("****************************** after update, do the registered collision status change?", self.registered_collisions)
        
        #wei: 在碰撞发生的时候 self.registered_collisions 会有值，但是在碰撞结束的时候，self.registered_collisions 会变成空的
        #不给self.registered_collisions赋新值，看多次碰撞发生的时候，这个变量是不是也是会变成是多值-- 结果是 新的碰撞事件会append 到self.registered_collisions里面
        #TODO: so how to record these events? or with more details?
        return new_status
    ...
    def terminate(self, new_status):
        ...
        Cleanup sensor
        """
        #wei: test the invalid status in record is because of the terminate function
        #print("~`~`~`~`~`~`~`~`~`~`~`~`~`~`~`~`~`~`~` collision test terminate function is called")
        
        if self._collision_sensor is not None:
            print("destroy the collision sensor")
            self._collision_sensor.destroy()
        self._collision_sensor = None
        
        #self._collision_sensor="jhh"
        #wei: here the json log collision sensor is Null is because of the analyze_scenario is called after the terminate function 
        #ans the collision_sensor is already destroyed
        
        # print("---------------------------------- actor in collision test", self.actor)
        # print("------------------------------------other actor in collision test", self.other_actor,self.other_actor_type)
    ...
    def _count_collisions(weak_self, event):     # pylint: disable=too-many-return-statements
        ...
        # print("---------------------------------------------- ego vehicle call the collision detection function")
        self = weak_self()
        if not self:
            return
        ...
        #FIXME：why the event is nonetype?  AttributeError: 'NoneType' object has no attribute 'id'
        #wei: 发生碰撞之后才report 的这个错误，所以没碰撞的时候应该是没有call this function 的
        # print("====================other_actor id is:",self.other_actor.id)
        # print("====================event.actor.id is:",event.other_actor.id) #test if the collision event exist
        ...
        if self.other_actor and self.other_actor.id != event.other_actor.id:
            return
        """
        # print("self.other_actor is:",self.other_actor)
        # print("self.other_actor.id is:",self.other_actor.id)
        # print("event.other_actor.id is:",event.other_actor.id)
        if self.other_actor and self.other_actor.id == event.other_actor.id:
            # print("-------------------------- EGO VEHICLE:collision with other actor")
            self.collision_with_other_actor = True
        # Filter to only a specific type
        # wei: from openscenario parser, it seems that the miscellaneous is all the static objects in the env
        # FIXME: change it to only contain the vehicle, we donot care about the static objects
        # Filter to only a specific type
        ...
        for collision_location in self.registered_collisions:
            ...
            if distance <= self.MIN_AREA_OF_COLLISION:
                return
        #TODO: wei: what is "traffic" actor here? 
        ...
        self.collision_event_dict.append(collision_event._dict)   
        ...
        #FIXME: 这个代码逻辑是没问题的，按理说如果调用了count_collision function,最后的log文件里这些部分不应该是空的? why and how to fix it ?



        
class NpcTest(Criterion):
    ...
    Args:
    - actor (carla.Actor): CARLA actor to be used for this test
    - other_actor (carla.Actor): only collisions with this actor will be registered TODO: change it to all the actors and register it, to see what will happened and change
    - other_actor_type (str): only collisions with actors including this type_id will count.
        Additionally, the "miscellaneous" tag can also be used to include all static objects in the scene
    - terminate_on_failure [optional]: If True, the complete scenario will terminate upon failure of this test
    - optional [optional]: If True, the result is not considered for an overall pass/fail result
    ...
    def __init__(self, actor, other_actor=None, other_actor_type=None,
                 optional=False, name="NpcTest", terminate_on_failure=False):
        """
        Construction with sensor setup 
        """
        
        super(NpcTest, self).__init__(name, actor, 0, None, optional, terminate_on_failure)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        ...
        self.other_actor = other_actor
        self.other_actor_type = other_actor_type
        self.registered_collisions_npc = []
        self.last_id = None
        self.collision_time = None
        ...
        # print("++++++++++++++++++++++++++++ actor in NPC test", self.actor)
        # print("++++++++++++++++++++++++++++ other actor in NPC test", self.other_actor)
    ...
    def update(self):
        ...
        # print("———————————————————————————————————————————————————————————————— after update, do the registered collision status change?",self.registered_collisions_npc,self.other_actor)
        #Wei: FIXED, Now the NPC test and collision test show the different collision object
        #FIXME: Now the problem becomes that how to fix the log file not shown what we want 
        return new_status
    ...
    def terminate(self, new_status):
        ...
        # print("++++++++++++++++++++++++++++ actor in NPC test", self.actor)
        # print("++++++++++++++++++++++++++++ other actor in NPC test", self.other_actor,self.other_actor.type_id)
        super(NpcTest, self).terminate(new_status)


    

    @staticmethod
    #TODO:是不是这里需要修改为普通的collision 判断条件
    def _count_collisions(weak_self, event):     # pylint: disable=too-many-return-statements
        """
        Callback to update collision count
        """
        # print("====================================== NPC call the collision detection function")
        ...
        # Filter to only a specific actor
        #忽略和不是指定 other actor 撞到的情况。
        #TODO: 可以再collision test 中测试着一块，因为ego vehicle 可以手动控制 
        
        #FIXME：这块看起来他们的代码逻辑是有点问题的，如果在这一步是撞到其他物体，就直接return 了，并不会到下面的标注actor_type 的地方
        """
        if self.other_actor and self.other_actor.id != event.other_actor.id:
            return
        """
        ...
        if self.other_actor and self.other_actor.id == event.other_actor.id:
            # print("====================================== NPC:collision with other actor")
            self.collision_with_other_actor = True
        
        # Filter to only a specific type
        #openscenario_parser 调用的时候并没有指定 other_actor_type 是什么。
        if self.other_actor_type:
            if self.other_actor_type == "miscellaneous":
                if "traffic" not in event.other_actor.type_id \
                        and "static" not in event.other_actor.type_id:
        ...
        # Ignore it if its too close to a previous collision (avoid micro collisions)--TODO: try to comment this part and see
        for collision_location in self.registered_collisions_npc:
        ...
        self.registered_collisions_npc.append(actor_location)
        self.list_traffic_events.append(collision_event)
